...
JustGage = function (config) {
    ...
    obj.config = {
        ...
        // useLevelColorForValue: bool
        // weather to use the guage color as the value font color
        useLevelColorForValue: kvLookup('useLevelColorForValue', config, dataset, false),
    ...
    obj.txtValue.attr({
        ...
        "fill": obj.config.useLevelColorForValue ? getColor(obj.config.value, (obj.config.value - obj.config.min) / (obj.config.max - obj.config.min), obj.config.levelColors, obj.config.noGradient, obj.config.customSectors) : obj.config.valueFontColor,
        "fill-opacity": "0"
    });
...
function getColor(val, pct, col, noGradient, custSec) {
    ...
    var noGradient = noGradient || custSec.length > 0;
        ...
        console.log([arguments, custSec.length, val]);
    if (typeof custSec !== 'undefined') {
